...
mod:hook_origin(CLASS.CraftingRerollPerkView, "_perform_crafting", function (self)
  ...
  craft_promise:next(function (results)
    ...
    local desired_perk = mod:get("rur_perk_name")

    -- if attempt_counter == 1 then
    --   mod:notify(safe_format("Rolling for rank " .. tostring(desired_rarity) .. " " .. tostring(desired_perk.text)))
    -- end
    if desired_perk == "none" then
      ...
      if new_perk.rarity >= desired_rarity and new_perk.id == desired_perk then
